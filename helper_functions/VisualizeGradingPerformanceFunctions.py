...
# Class to create the training result DataFrame by merging the test result and real result DataFrames
class TrainingResultCreator:
    def __init__(self, test_result_path, real_result_path):
        """
        Initialize the TrainingResultCreator with paths to the test result and real result files.
        
        Parameters:
        test_result_path (str): Path to the test result CSV file.
        real_result_path (str): Path to the real result CSV file.
        """
        self.test_result_path = test_result_path
        self.real_result_path = real_result_path

    def create_training_result_df(self):
        """
        Create the training result DataFrame by merging the test result and real result DataFrames.
        
        Returns:
        pd.DataFrame: The merged training result DataFrame.
        """
        # Read the test result file and get the pass/fail DataFrame
        y_pred = SectionGradeAggregator(self.test_result_path, "AI").get_fail_pass_df()
        
        # Read the real result file and drop the first column
        y_test = pd.read_csv(self.real_result_path)
        y_test = y_test.drop(y_test.columns[0], axis=1)
        
        # Merge the test result and real result DataFrames
        training_result_df = pd.merge(y_pred, y_test, how='left', on=['filename', 'Section'])
        
        return training_result_df
...
class ModelResultAnalyzer():
    ...
    def set_threshold(self,threshold):
        ...
        self.training_result['pass_AI'] = (self.training_result['AI_Grade'] >= self.training_result['size']).astype(int)
        self.training_result['pass_Real'] = (self.training_result['Real_Grade'] >= self.training_result['size']).astype(int)

    def plot_confusion_matrix(self,section=""):
        # self.set_threshold(self.pass_threshold)
